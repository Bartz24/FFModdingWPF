...
public class EnemyRando : Randomizer
{
    ...
    {
        Dictionary<string, DataStoreWDB<DataStoreBtCharaSpec>> dbs = orig ? enemiesOrig : enemies;
        return dbs.Values.SelectMany(db3 => db3.Values.Where(e => e.record == id)).First();
    }
    public bool HasEnemy(string id)
    {
        return enemies.Values.SelectMany(db3 => db3.Values.Where(e => e.record == id)).Count() > 0;
    }

...
public class APEquipRando : EquipRando
{
    ...
    public override void Load()
    {
        ...
        // Create unique AP key items based on FF13_2ArchipelagoData item placements
        ...
        foreach (var placement in apData.ItemPlacements)
        {
            ...
            {
                var apUnique = items.Copy("key_l_knife", itemId);
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            if (!items.Keys.Contains(itemId))
            {
                var apMulti = items.Copy("key_l_knife", itemId);
        ...
        {
            var apAdded = items.Copy("key_l_knife", "key_r_added");

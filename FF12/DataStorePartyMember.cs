...
namespace Bartz24.FF12
{
    public class DataStorePartyMember : DataStore
    {
        ...
        {
            get { return (RecalculateLevelLPType)Data.ReadByte(0x2C, 3, 2); }
            set { Data.SetByte(0x2C, (byte)value, 3, 2); }
        }
        public byte Level
        {
            get { return Data.ReadByte(0x2E); }
        ...
        {
            get { return Enumerable.Range(0, 10).Where(i => Data.ReadUShort(0x58 + i * 2) != 0xFFFF).Select(i => Data.ReadByte(0x34 + i)).ToList(); }
            set { Enumerable.Range(0, 10).ForEach(i => Data.SetByte(0x34 + i, i >= value.Count ? (byte)0 : value[i])); }
        }
        public List<ushort> ItemIDs
        {
            get { return Enumerable.Range(0, 10).Select(i => Data.ReadUShort(0x58 + i * 2)).Where(a => a != 0xFFFF).ToList(); }

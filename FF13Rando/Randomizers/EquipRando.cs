...
public class EquipRando : Randomizer
{
    ...
    private void RandomizeEquipmentPassivesAndStats()
    {
        ...
        HashSet<DataStoreEquip> equipHitPassiveCap = new();
            ...
            if (FF13Flags.Equip.RandEquipStats.FlagEnabled)
            {
                ...
                if (e.i8StrengthInitial > 0 && e.i8MagicInitial > 0)
                {
                    ...
                    if (parent == null || GetRandomPassiveRoot(passive) != GetRandomPassiveRoot(parentPassive))
                    {
                        ...
                        int[] zeros = { 5, 5 };
                        int[] negs = { 0, 0 };
                        StatPoints statPoints = new(bounds, weights, chances, zeros, negs, 0.5f);
    ...
    private PassiveData GetRandomPassiveRoot(PassiveData p)
    {
        ...
        return p == parent ? p : GetRandomPassiveRoot(parent);
    }
    ...
    public override Dictionary<string, HTMLPage> GetDocumentation()
    {
        ...
        List<string> GenerateRowContents(DataStoreEquip e)
        {
            ...
            return itemData[e.ID].Category == "Accessory"
                ? new string[] { name, passive, ((SynthesisGroup)items[e.ID].SynthesisGroup).ToString().SeparateWords() }.ToList()
                : new string[] { name, strength, magic, passive, ((SynthesisGroup)items[e.ID].SynthesisGroup).ToString().SeparateWords() }.ToList();
        }
    }

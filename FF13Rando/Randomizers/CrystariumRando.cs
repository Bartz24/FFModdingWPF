...
public class CrystariumRando : Randomizer
{
    ...
    private void RandomizeAbilities()
    {
        foreach (string chara in chars)
        {
            ...
            do
            {
                ...
                List<DataStoreCrystarium> remaining = new(abilityNodes);
                List<string> abilitiesPlaced = new();
    ...
    private void UpdateCPCosts()
    {
        ...
        {
            costs.Add("lightning", new uint[] { 75, 90, 380, 145, 420, 1000, 970, 6000, 12000, 45000 });
            costs.Add("sazh", new uint[] { 75, 90, 380, 145, 420, 1000, 970, 6000, 12000, 45000 });
            costs.Add("hope", new uint[] { 80, 90, 130, 275, 480, 1080, 915, 6000, 12000, 45000 });
            costs.Add("vanille", new uint[] { 70, 75, 320, 130, 400, 1040, 775, 6000, 12000, 45000 });
            costs.Add("snow", new uint[] { 40, 200, 200, 200, 400, 1050, 865, 6000, 12000, 45000 });
            costs.Add("fang", new uint[] { 70, 80, 350, 220, 450, 1200, 775, 6000, 12000, 45000 });
        }
    ...
    private void UpdateNodeCPCost(string first, string chara, uint[] costs, string[] primaries)
    {
        uint[] secondaryCosts = { 3000, 6000, 9000, 12000, 15000, 18000, 21000, 24000, 30000, 60000 };
        string[] roles = { "sen", "com", "rav", "syn", "sab", "med" };

        Func<DataStoreCrystarium, uint[]> getCostArray = c =>
        {
            return primaries != null && !primaries.Contains(roles[(int)(c.iRole - 1)]) ? secondaryCosts : costs;
        };
        ...
        crystariums[chara].Values.ForEach(c =>
        {
            if (c.iStage == 1)
            {
                c.iCPCost = (int)c.iRole - 1 == roles.ToList().IndexOf(first) ? c.ID != firstNodes[chara][c.iRole] ? getCostArray(c)[0] : 0 : getCostArray(c)[0];
            }
            else
            {
                c.iCPCost = getCostArray(c)[c.iStage - 1];
            }
        });
    }
    ...
    {
        Dictionary<Role, float[]> roleMults = new();
        for (int r = 1; r <= 6; r++)
        {
            Role role = (Role)r;
            StatPoints statPoints;
        ...
        foreach (string chara in chars)
        {
            List<CrystariumType> types = new() { CrystariumType.HP, CrystariumType.Strength, CrystariumType.Magic };
    ...
    private void ShuffleNodesBetweenRoles()
    {
        foreach (string chara in chars)
        {
            List<CrystariumType> types = new() { CrystariumType.Accessory, CrystariumType.ATBLevel, CrystariumType.HP, CrystariumType.Strength, CrystariumType.Magic };
    ...
    public override Dictionary<string, HTMLPage> GetDocumentation()
    {
        ...
        HTMLPage page = new("Crystarium", "template/documentation.html");
        ...
        chars.ForEach(name =>
        {
            ...
            {
                int hpTotal = roleGroup.Where(c => c.iType == CrystariumType.HP).Sum(c => c.iValue);
                int strengthTotal = roleGroup.Where(c => c.iType == CrystariumType.Strength).Sum(c => c.iValue);
                int magicTotal = roleGroup.Where(c => c.iType == CrystariumType.Magic).Sum(c => c.iValue);
            ...
            static string writeStatsForDisplay((int, int, int) input)
            {
                return string.Join("<br/>", $"HP + {input.Item1}", $"Strength + {input.Item2}", $"Magic + {input.Item3}");
            }
            ...
            page.HTMLElements.Add(new Table(name[0].ToString().ToUpper() + name.Substring(1),
                ...
                Enumerable.Range(1, 10).Select(stage =>
                {
                    ...
                    int strengthTotal = 0;
                    int magicTotal = 0;
                    List<string> totalAdditions = new();
    ...
    private void ApplyCPCostModifiers()
    {
        foreach (string chara in chars)
        {
            if (FF13Flags.Stats.CPCostMult.FlagEnabled)
            {
                crystariums[chara].Values.Where(c => c.iCPCost > 0).ForEach(c =>
                {
                    c.iCPCost = (uint)Math.Max(c.iCPCost * FF13Flags.Stats.CPCostMultValue.Value / 100, 5);
                });
            }
        ...
        foreach (string chara in chars)
        {
            crystariums[chara].Values.ForEach(c =>
            {
                if (c.iCPCost > 0)
                {
                    ...
                    {
                        interval = c.iCPCost < 1000 ? 10 : c.iCPCost < 5000 ? 250 : c.iCPCost < 10000 ? 500 : c.iCPCost < 50000 ? 1000 : 5000;
                    }
